...
{
  options.ceeser.machines.server = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = ''
        Determines whether a machine is a server and installs / activates server packages and services
      '';
    };
  };

  config = lib.mkIf config.ceeser.machines.server.enable {
    ...
    ceeser.profiles.headless.enable = true;
    ceeser.profiles.minimal.enable = true;
    ceeser.services.pulldotfiles.enable = true;
